...
func (m *HARViewModel) renderTitle() string {
	title := fmt.Sprintf(" HAR Viewer - %s ", m.fileName)
	...
	return titleStyle.Render(title) + countStyle.Render(entryCount)
}
...
func (m *HARViewModel) renderStatusBar() string {
	...
	if m.viewMode == ViewModeTable {
		...
		parts = append(parts, "↑/↓: Navigate Table")
	...
	if m.selectedIndex < len(m.allEntries) {
		entry := m.allEntries[m.selectedIndex]
		info := fmt.Sprintf("Entry %d/%d", m.selectedIndex+1, len(m.allEntries))
		parts = append(parts, info)

		if entry.URL != "" {
			urlInfo := truncateString(entry.URL, maxURLDisplayLength)
			parts = append(parts, urlInfo)
		}
	}
...
func (m *HARViewModel) renderSplitPanel() string {
	...
	leftPanel := borderStyle.
		Width(panelWidth).
		Height(m.height/2-tableVerticalPadding).
		Render(m.requestViewport.View())
